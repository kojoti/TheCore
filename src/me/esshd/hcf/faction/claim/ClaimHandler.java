...
public class ClaimHandler {
	...
	public static final ItemStack SUBCLAIM_WAND = new ItemBuilder(Material.GOLD_SPADE, 1)
			.displayName(ChatColor.GOLD + "Subclaim Wand")
			.lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Set the first and second position of ",
					ChatColor.GRAY + "your Subclaim selection.", "", ChatColor.AQUA + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.AQUA + " to:",
					ChatColor.GRAY + "Clear your current Subclaim selection.", "", ChatColor.AQUA + "Use " + ChatColor.YELLOW + "/faction subclaim create <name>" + ChatColor.AQUA + " to:",
	...
	public static final ItemStack CLAIM_WAND = new ItemBuilder(Material.DIAMOND_HOE)
			.displayName(ChatColor.RED + "Claim Wand")
			.lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:", ChatColor.GRAY + "Set the first and second position of ",
					ChatColor.GRAY + "your Claim selection.", "", ChatColor.AQUA + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.AQUA + " to:",
	...
	public static final int MAX_CHUNKS_PER_LIMIT = ConfigurationService.MAXCHUNKSPERCLAIM;
	...
	/**
	 * Checks if a {@link Player} is eligible to {@link Subclaim} at a given {@link Location}.
	 *
	 ...
	 * 
        ...
        World world = location.getWorld();
        if(player.isOp()){
        	return true; <-- bypasses all claim preventer things idk how to explain
        }
        if (world.getEnvironment() != World.Environment.NORMAL) {
        ...
        if (!(this.plugin.getFactionManager().getFactionAt(location) instanceof WildernessFaction)) {
            player.sendMessage((Object)ChatColor.RED + "You can only claim land in the " + (Object)ConfigurationService.WILDERNESS_COLOUR + "Wilderness" + (Object)ChatColor.RED + ". " + "Make sure you are past " + 750 + " blocks from spawn..");
            return false;
        }
	...
	public boolean tryPurchasing(Player player, Claim claim) {
		...
		if (!HCF.getPlugin().bypassing.contains(player.getUniqueId())) {
			...
			if (claimPrice > factionBalance) {
				player.sendMessage(ChatColor.RED + "Your faction bank only has " + EconomyManager.ECONOMY_SYMBOL + factionBalance + ", the price of this claim is " + EconomyManager.ECONOMY_SYMBOL
						+ claimPrice + '.');

				return false;
			}

			if (claim.getChunks().size() > ConfigurationService.MAXCHUNKSPERCLAIM) {
				player.sendMessage(ChatColor.RED + "Claims cannot exceed " + ConfigurationService.MAXCHUNKSPERCLAIM + " chunks.");
				return false;
			}
			...
			player.sendMessage(ChatColor.GOLD + "Claim has been purchased for " + ChatColor.GREEN + EconomyManager.ECONOMY_SYMBOL + claimPrice + ChatColor.GOLD + '.');
			playerFaction.setBalance(factionBalance - claimPrice);
			playerFaction.broadcast(ChatColor.GOLD + player.getName() + ChatColor.GREEN + " claimed land for your faction at " + ChatColor.GOLD + '(' + center.getBlockX() + ", " + center.getBlockZ()
			...
			if (!playerFaction.addClaim(claim, player))
				return false;
			Location center = claim.getCenter();
			player.sendMessage(ChatColor.GOLD + "Claim has been purchased with bypass for " + ChatColor.GREEN + EconomyManager.ECONOMY_SYMBOL + claimPrice + ChatColor.GOLD + '.');
			playerFaction.setBalance(factionBalance - claimPrice);
			playerFaction.broadcast(ChatColor.GOLD + player.getName() + ChatColor.GREEN + " claimed land for your faction at " + ChatColor.GOLD + '(' + center.getBlockX() + ", " + center.getBlockZ()
			+ ')' + ChatColor.GREEN + '.', player.getUniqueId());
