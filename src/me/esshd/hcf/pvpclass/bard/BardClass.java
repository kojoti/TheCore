...
public class BardClass extends PvpClass implements Listener {
    ...
    public static final int HELD_EFFECT_DURATION_TICKS = 100; // the amount of
    // time in ticks
    // to apply a
    // Held potion
    // effect for
    ...
    private static final long BUFF_COOLDOWN_MILLIS = TimeUnit.SECONDS.toMillis(10L); // time
    // in
    // milliseconds
    // for
    // Bard
    ...
    public BardClass(HCF plugin) {
        ...
        this.bardEffects.put(Material.WHEAT, new BardEffect(20, new PotionEffect(PotionEffectType.SATURATION, 120, 1),
                new PotionEffect(PotionEffectType.SATURATION, 120, 0)));
    ...
    public boolean onEquip(Player player) {
        ...
        bardData.heldTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (held != null) {
                    ...
                    if (!plugin.getFactionManager().getFactionAt(player.getLocation()).isSafezone()) {
                        ...
                        if (playerFaction != null) {
                            Collection<Entity> nearbyEntities = player.getNearbyEntities(TEAMMATE_NEARBY_RADIUS,
                                    TEAMMATE_NEARBY_RADIUS, TEAMMATE_NEARBY_RADIUS);
                ...
                if (energy != 0 && energy != lastEnergy
                        && (energy % 10 == 0 || lastEnergy - energy - 1 > 0 || energy == BardData.MAX_ENERGY)) {
                    lastEnergy = energy;
                    player.sendMessage(ChatColor.GRAY + "* " + ChatColor.GOLD + name + ChatColor.YELLOW + " energy is now at "
        ...
        if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
            ...
            if (bardData != null) {
                ...
                if (bardEffect != null
                    ...
                    if (playerFaction != null
                            && !bardEffect.clickable.getType().equals((Object) PotionEffectType.WITHER)) {
                        final Collection<Entity> nearbyEntities = (Collection<Entity>) player.getNearbyEntities(25.0,
                                25.0, 25.0);
                ...
                player.sendMessage(ChatColor.YELLOW + "You have just used " + name + " buff " + ChatColor.RED
                        + Lang.fromPotionEffectType(bardEffect.clickable.getType()) + ' '
                        ...
                        + "Your Energy is now " + ChatColor.GREEN
                        + ((newEnergy * 10.0) / 10.0)/* TODO:neeeded? */ + ChatColor.YELLOW + '.');
            }
        }
    }
