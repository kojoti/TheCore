...
public class SpawnCannonCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (location.getBlock().getRelative(BlockFace.DOWN).getType() != SPAWN_CANNON_BLOCK) {
            sender.sendMessage(ChatColor.RED + "You are not on a spawn cannon (" + ChatColor.WHITE
                    + SPAWN_CANNON_BLOCK.name() + ChatColor.RED + ").");
            return true;
        }
    ...
    public void launchPlayer(Player player, Location location) {
        ...
        if (Math.abs(x) > maxDistance || Math.abs(z) > maxDistance) {
            player.sendMessage(ChatColor.RED + "You cannot launch that far from the spawn cannon. Your limit is "
                    + maxDistance + '.');
            return;
        }

        location = BukkitUtils.getHighestLocation(location).add(0, 3, 0);
        player.sendMessage(ChatColor.GOLD + "Cannoning to " + ChatColor.WHITE + x + ", " + z + ChatColor.GOLD + '.');
