...
import me.esshd.api.utils.NmsUtils;
...
public class EnderPearlTimer extends PlayerTimer implements Listener {
    ...
    public void onExpire(UUID userUUID) {
        ...
        if (player != null) {
            player.sendMessage(ChatColor.GREEN + "You may now use an enderpearl again.");
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.hasItem() && event.getItem().getType() == Material.ENDER_PEARL && (event.getAction() == Action.RIGHT_CLICK_AIR)) {
            Player player = event.getPlayer();
            long remaining = getRemaining(player);
            if (remaining > 0L) {
                player.sendMessage(ChatColor.GOLD.toString() + "Enderpearl Cooldown" + ChatColor.GRAY + ": " + ChatColor.GRAY
        ...
        public void run() {
            ...
            if (stack != null && stack.getType() == Material.ENDER_PEARL) {
                ...
                if (remaining > 0L) {
                    ...
                    item.c(ChatColor.YELLOW + "Enderpearl Cooldown" + ChatColor.GOLD + ": " + ChatColor.WHITE + DurationFormatter.getRemaining(remaining, true, true));
