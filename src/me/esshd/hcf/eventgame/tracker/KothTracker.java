...
public class KothTracker implements EventTracker {
    ...
    public void tick(EventTimer eventTimer, EventFaction eventFaction) {
        ...
        if (remainingSeconds > 0 && remainingSeconds % 30 == 0) {
            Bukkit.broadcastMessage(ChatColor.GOLD + "[KoTH] " + ChatColor.YELLOW + "Someone is controlling " + ChatColor.RED
    ...
    public void onContest(EventFaction eventFaction, EventTimer eventTimer) {
        Bukkit.broadcastMessage(ChatColor.GOLD + "[KoTH] " + ChatColor.RED + eventFaction.getName() + ChatColor.YELLOW + " can now be contested. "
    ...
    public void onControlLoss(Player player, CaptureZone captureZone, EventFaction eventFaction) {
        player.sendMessage(ChatColor.RED + "You are no longer in control of " + ChatColor.RED + captureZone.getDisplayName() + ChatColor.RED + '.');
        ...
        if (remainingMillis > 0L && captureZone.getDefaultCaptureMillis() - remainingMillis > MINIMUM_CONTROL_TIME_ANNOUNCE) {
            Bukkit.broadcastMessage(ChatColor.GOLD + "[KoTH] " + ChatColor.RED + captureZone.getDisplayName() + ChatColor.YELLOW + " has been knocked!"
