    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        for (Map.Entry<LocalDateTime, String> entry : scheduleMap.entrySet()) {
            ...
            if (scheduleDateTime.isAfter(now)) { // only show the events that haven't been scheduled yet.
                ...
                shownEvents.add("  " + colour + WordUtils.capitalizeFully(entry.getValue()) + ": " + ChatColor.YELLOW + weekName + ' ' + scheduleDateTime.getDayOfMonth() + ' ' + monthName
                        + ChatColor.RED + " (" + HHMMA.format(scheduleDateTime) + ')' + ChatColor.GRAY + " - " + ChatColor.GOLD
        ...
        sender.sendMessage(ChatColor.GRAY + "Server time is currently " + ChatColor.WHITE + weekName + ' ' + now.getDayOfMonth() + ' ' + monthName + ' ' + HHMMA.format(now) + ChatColor.GRAY + '.');
