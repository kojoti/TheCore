...
import me.esshd.hcf.faction.event.CaptureZoneEnterEvent;
import me.esshd.hcf.faction.event.CaptureZoneLeaveEvent;
...
public class ProtectionListener implements Listener {
    ...
    private static final ImmutableMultimap<Material, Material> ITEM_BLOCK_INTERACTABLES = ImmutableMultimap.<Material, Material>builder().put(Material.DIAMOND_HOE, Material.GRASS)
            .put(Material.GOLD_HOE, Material.GRASS).put(Material.IRON_HOE, Material.GRASS).put(Material.STONE_HOE, Material.GRASS).put(Material.WOOD_HOE, Material.GRASS).build();

    private static final ImmutableSet<Material> BLOCK_INTERACTABLES = Sets.immutableEnumSet(Material.BED, Material.BED_BLOCK, Material.BEACON, Material.FENCE_GATE, Material.IRON_DOOR,
            Material.TRAP_DOOR, Material.WOOD_DOOR, Material.WOODEN_DOOR, Material.IRON_DOOR_BLOCK, Material.CHEST, Material.TRAPPED_CHEST, Material.FURNACE, Material.BURNING_FURNACE,
            Material.BREWING_STAND, Material.HOPPER, Material.DROPPER, Material.DISPENSER, Material.STONE_BUTTON, Material.WOOD_BUTTON, Material.ENCHANTMENT_TABLE, Material.ANVIL,
            Material.LEVER, Material.FIRE);
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getBlock().getLocation(), ChatColor.YELLOW + "You may not build in the territory of %1$s" + ChatColor.YELLOW + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getBlockPlaced().getLocation(), ChatColor.YELLOW + "You may not build in the territory of %1$s" + ChatColor.YELLOW + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onBucketFill(PlayerBucketFillEvent event) {
        if (!attemptBuild(event.getPlayer(), event.getBlockClicked().getLocation(), ChatColor.YELLOW + "You may not build in the territory of %1$s" + ChatColor.YELLOW + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onHangingInteractByPlayer(PlayerInteractEntityEvent event) {
        ...
        if (entity instanceof Hanging) {
            if (!attemptBuild(event.getPlayer(), entity.getLocation(), ChatColor.YELLOW + "You may not build in the territory of %1$s" + ChatColor.YELLOW + '.')) {
                event.setCancelled(true);
            }
        }
    }
    
    /*public void onPlayerInteractByPlayer(PlayerInteractEvent event) {
    Faction factionAt = plugin.getFactionManager().getFactionAt(((BlockState) event).getBlock().getLocation());
    if (factionAt instanceof KothFaction && !(factionAt instanceof PlayerFaction)) {
    //Player player = e.getPlayer();
    if(event.getAction.equals(Action.RIGHT_CLICK_AIR) || event.getPlayer().getAction.equals(Action.RIGHT_CLICK_BLOCK)) {
    if(event.getPlayer().getItemInHand().getType().equals(Material.ENDER_PEARL)) {
    event.setCancelled(true);*/


    /**
     * Checks if a entity is eligible to build at a given location, if not it will send the deny message passed in the constructor.
     * <p>
     * The deny message will be formatted using {@link String#format(String, Object...)}
     * </p>
     * <p>
     * The first formatted argument is the display name of the enemy faction to the player
     * </p>
     *
    ...
    public static boolean attemptBuild(Entity entity, Location location, @Nullable String denyMessage, boolean isInteraction) {
        ...
        if (player != null && player.getWorld().getEnvironment() == World.Environment.THE_END) {
            player.sendMessage(ChatColor.RED + "You may not build in the end.");
            return false;
        }
        ...
        if (result) {
            ...
            if (!isInteraction && factionAt instanceof WarzoneFaction) {
                if (denyMessage != null && player != null) {
                    //player.sendMessage(ChatColor.YELLOW + "You may not build in the " + factionAt.getDisplayName(player) + ChatColor.YELLOW + ".");
                }
